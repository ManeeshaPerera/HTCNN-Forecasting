...
from constants import HORIZON_SWIS, SWIS_POSTCODES, ALL_SWIS_TS, ARIMA_FAILURES, categories
...
def run_naive_approach():
    ...
    for category in categories:
        for sample in range(0, 10):
            ts = f'grid_sample_{sample}'
            print("running ts ==>", ts)
            pc_data = pd.read_csv(f'swis_ts_data/{category}/{ts}.csv', index_col=0)
            ...
            forecasts.to_csv(f'swis_ts_data/{category}/naive/{ts}.csv')
