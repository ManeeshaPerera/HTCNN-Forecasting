...
from constants import SWIS_POSTCODES, Clusters, PC_CLUSTER_MAP
...
def SWIS_APPROACH_A_more_layer_without_norm_weather_centroids():
    input_layers_pc = []
    cluster_map = []
    for ts in SWIS_POSTCODES:
    ...
    input_layers_pc.append(grid_model.input)
    SWIS_APPROACH_A_more_layer_without_norm_weather_centroids_model = keras.Model(
        inputs=input_layers_pc, outputs=prediction_layer)

    SWIS_APPROACH_A_more_layer_without_norm_weather_centroids_model.compile(loss=tf.losses.MeanSquaredError(),
                                                          optimizer=tf.optimizers.Adam(0.0001),
                                                          metrics=[tf.metrics.MeanAbsoluteError()])
    return SWIS_APPROACH_A_more_layer_without_norm_weather_centroids_model