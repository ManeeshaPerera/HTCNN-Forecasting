...
from src.CNN_architectures.swis_new_architectures import grid_conv_in_each_pc_seperately
...
def get_samples(map_dic):
    ...
    for key_dic, map_data in map_dic.items():
        if key_dic == 'grid':
            data_grid, _ = create_numpy_arrays(map_data, require_labels=True)
            ...
            output_labels['label_pc'] = label_pc
    return output_values, output_labels['label_pc']


def run_combine_model(approach, ts_index):
    window_data_pc = {}
    window_data_grid = {}
    pc_value = SWIS_POSTCODES[ts_index]

    for swis_ts in ['grid', pc_value]:
        if swis_ts == 'grid':
    ...
    train_dic, label_pc = get_samples(map_dic_train)
    test_dic, label_pc_test = get_samples(map_dic_test)

    model = approach(pc_value)
    callback = tf.keras.callbacks.EarlyStopping(monitor='loss', patience=50)
    history = model.fit(train_dic, label_pc, batch_size=128, epochs=1000,
                        callbacks=[callback], shuffle=False)

    # Forecast
    lookback = 1
    data = pd.read_csv(f'swis_ts_data/ts_data/{pc_value}.csv', index_col=[0])
    look_back = 18 * lookback
...
final_test_models = {'0': {'func': grid_conv_in_each_pc_seperately,
                           'model_name': 'grid_conv_in_each_pc_seperately',
                           'folder': 'new_models'},
                     }
...
model_new_name = f'{model_name}/{SWIS_POSTCODES[timeseries]}/{run}'
forecasts, history = run_combine_model(function_run, timeseries)
