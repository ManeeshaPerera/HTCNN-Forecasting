...
from constants import UNIQUE_WEATHER_PCS

from src.CNN_architectures.swis_new_architectures import SWIS_APPROACH_A_more_layer_without_norm_grid_skip, \
...
def create_window_data(filename, lookback=1):
    ...
    if filename == 'grid':
        ...
        # if pc we only take the weather
        data = pd.read_csv(f'swis_ts_data/ts_data/{filename}.csv', index_col=[0]).iloc[:, 7:]
    print(filename)
...
def run_combine_model(approach):
    ...
    for swis_ts in UNIQUE_WEATHER_PCS:
        if swis_ts == 'grid':
...
final_test_models = {'0': {'func': SWIS_APPROACH_A_with_weather_only,
                           'model_name': 'SWIS_APPROACH_A_with_weather_only',
                           'folder': 'new_models'},
                     }
